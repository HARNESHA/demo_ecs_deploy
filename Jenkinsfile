    ...
    stages {
        ...
        stage('Push') {
            steps {
                ...
                echo "----------------------------------------------------------------------------"
                sh ''' docker login -u AWS -p $(aws ecr get-login-password --region ap-south-1 --profile iam-jay ) 332289956654.dkr.ecr.ap-south-1.amazonaws.com '''
                //sh 'docker login --username AWS --password $(aws ecr get-login-password --region ap-south-1 --profile iam-jay ) 332289956654.dkr.ecr.ap-south-1.amazonaws.com'            }
                /* sh '''
